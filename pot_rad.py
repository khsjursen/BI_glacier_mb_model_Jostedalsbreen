...
Functions for calculating potential direct incoming solar radiation in each
...
def calc_pot_rad(da_dem_elevation):
    ...
    # Alternative cos_theta calculation: 
    # takes approx 8.1s:
    # cos_theta = (np.cos(delta) * sin(lat_mean_rad) 
    #               * np.cos(slope_rad) 
    #               + np.sin(delta) * cos(lat_mean_rad) 
    #               * np.multiply(np.sin(slope_rad), np.cos(aspect_rad)) 
    #               + np.cos(delta) * cos(lat_mean_rad) 
    #               * np.cos(slope_rad) * np.cos(hra) 
    #               - np.cos(delta) * sin(lat_mean_rad) 
    #               * np.multiply(np.sin(slope_rad), np.cos(aspect_rad)) 
    #               * np.cos(hra) 
    #               - np.cos(delta) * sin(lat_mean_rad) 
    #               * np.sin(slope_rad) * np.sin(hra))
    # takes approx 2.7s:
    # cos_theta = ne.evaluate("""
    #     cos(delta) * sin(lat_mean_rad) * cos(slope_rad) 
    #     + sin(delta) * cos(lat_mean_rad) * sin(slope_rad) * cos(aspect_rad) 
    #     + cos(delta) * cos(lat_mean_rad) * cos(slope_rad) * cos(hra) 
    #     - cos(delta) * sin(lat_mean_rad) * sin(slope_rad) * cos(aspect_rad) * cos(hra) 
    #     - cos(delta) * sin(lat_mean_rad) * sin(slope_rad) * sin(hra)
    #     """.replace("\n", "").replace(" ","")
    #     ) 
    ...
    # Calculate the potential direct solar radiation in each cell for the 
    # each hour and day of a year that is between sunrise and sunset.
    # takes approx. 7.4s:
    # pot_rad_hr = (I_0 * ecc_corr_factor * np.multiply(
    #                       np.power(psi_a, (p / (p_0 * np.cos(zenith_angle)))), 
    #                       cos_theta))
    # takes approx. 1.8s:
    #pot_rad_hr = ne.evaluate("I_0 * ecc_corr_factor * psi_a **(p / (p_0 * cos(zenith_angle))) * cos_theta")
...
# figure1 = plt.figure(dpi=300) #figsize=[10,6])
# ax = figure1.gca()
# #plt.imshow(pot_rad_sum[180,:,:])
# plt.imshow(pot_rad_sum1[180,:,:])
# #ax.add_patch(PolygonPatch(pgon, fill=False, color='red', linewidth = 0.5))
# plt.colorbar()
# plt.show()
