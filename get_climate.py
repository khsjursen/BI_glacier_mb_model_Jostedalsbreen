...
correction of precipitation and temperature data from the seNorge model height
to the surface elevation of the glacier.
...
def get_climate(Y_coordinates, X_coordinates, mask_ca_1, year: str, name_dir_files: dict, rcp, climate_model='kss', return_model_h=False):
    
    """
    The function get_climate() returns numpy arrays of raw temperature
    and precipitation data from climate dataset for the given year. 
    
    Function get_climate() returns xarray.Dataset with climate data
    (precipitation and temperature) from a climate dataset for a 
    specific area and year.
    
    If climate data for the specific area (given by glacier name) and year is
    available on file, the file is loaded as a Dataset and returned. If the
    data is not available on file the netCDF file containing the seNorge_2018 
    dataset for the particular year is read from thredds.met.no as an xarray 
    DataSet. The data for the specified area is selected, saved to netCDF file
    and returned. 
    ...
    mask_ca_1: numpy.array
        Mask of glacier/catchment area with ones for cells part of glacier/
        catchment and zeros otherwise.
    year : str
    ...
    name_dir_files : dict
        ...
        climate files.
    rcp: str
        RCP scenario for running future simulations. 'rcp45' or 'rcp85', for 
        any other value historical climate is retrieved.
    climate_model: str
        Source of climate data. 'kss' or 'seNorge' for Norwegian Center for 
        Climate Services or SeNorge.
    return_model_h : bool
    ...
    if rcp=='hist':
        ...
        filename = name_dir_files['filename_climate_data'] + '_'+rcp+'_' + year + '.nc'
        # Name of numpy file with temperature to load or store.
        filename_temp = name_dir_files['filepath_temp_prec_raw'] + 'temp_sn_'+rcp+'_' + year + '.npy'
        # Name of numpy file with precipitation to load or store.
        filename_prec = name_dir_files['filepath_temp_prec_raw'] + 'prec_sn_'+rcp+'_' + year + '.npy'

    # Name of numpy file with elevation of seNorge model.
    filename_elev = name_dir_files['filepath_temp_prec_raw'] + 'elev_sn.npy'

    if climate_model == 'seNorge':
        ...
        filepath_dem = name_dir_files['filepath_dem_base'] + name_dir_files['filename_dem_base']
    ...
    # inside coordinates of the center of the corner cells (with buffer) in 
    # ds_geo and save as filename in filepath.
    while not os.path.isfile(filepath_clim_data + filename):
        
        if climate_model == 'kss':
            # Get KSS climate data source file (must be added manually to folder and 
            # updated depending on if hist, rcp45 or rcp85 is used).
            
            if rcp=='rcp45':
                filepath = filepath_clim_data + 'rcp45_ECEARTH_CCLM_' + year + '.nc'
            elif rcp=='rcp85':
                filepath = filepath_clim_data + 'rcp85_ECEARTH_CCLM_' + year + '.nc'
            else:
                filepath = filepath_clim_data + 'hist_ECEARTH_CCLM_' + year + '.nc'
        else:
        ...
        with xr.open_dataset(filepath_dem, engine='netcdf4') as ds_model_dem:
            ...
            if climate_model == 'kss':
                ds_model_dem = ds_model_dem.rename({'elevation':'model_height'})
                ds_model_dem = ds_model_dem.drop_vars(["crs"])
                ds_model_dem = ds_model_dem.drop_vars(["lat","lon"])
                # Fill zeros in cells with nan (ocean/fjords)
                ds_model_dem = ds_model_dem.fillna(0)
        ...
        with xr.open_dataset(filepath, engine='netcdf4') as ds_clim:
            ...
            if climate_model == 'kss':
                ...
                if rcp=='rcp45':
                    ds_clim = ds_clim.drop_vars(["projection_utm",
                                                 "air_temperature__map_rcp45_daily",
                                                 "lat", "lon"])   
                    
                elif rcp=='rcp85':
                    ds_clim = ds_clim.drop_vars(["projection_utm",
                                                 "air_temperature__map_rcp85_daily",
                                                 "lat", "lon"])   
                else:
                    ds_clim = ds_clim.drop_vars(["projection_utm",
                                                 "air_temperature__map_hist_daily",
                                                 "lat", "lon"]) 
    ...
    while not os.path.isfile(filename_temp):
        ...
        with xr.open_dataset(filepath_clim_data + filename) as ds_clim_cropped:
            ...
            # Array of temperature masked with cells part of catchment. All other
            # cells are nan. 
